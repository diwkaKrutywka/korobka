...
function wrapText(text, max = 24) {
	...
	const lines = [];
	let current = "";
	for (const word of words) if ((current + " " + word).trim().length > max) {
...
function buildTicketLines(p, cfg) {
	...
	if (cfg.clinicAddress) push({
		text: cfg.clinicAddress,
		center: true,
		size: 11
	});
	push({ lf: 1 });
	...
	const block = (label, value) => {
		...
		wrapText(value, 20).forEach((line) => push({
			text: line,
			size: 14
		}));
		push({ lf: 1 });
	...
	if (p.cabinet) {
		const cabVal = p.floor ? `${p.cabinet}, ${p.floor} ${L.floorSuffix}` : p.cabinet;
	...
	push({ lf: 1 });
	if (cfg.clinicPhone) push({
		text: `${L.registry} · ${cfg.clinicPhone}`,
		center: true,
		size: 11
	});
	if (cfg.clinicSite) push({
		text: cfg.clinicSite,
		center: true,
		size: 11
	});
...
function startPs() {
	...
	psProc.stdout.on("data", (chunk) => {
		...
		for (const raw of lines) {
			...
			try {
				...
				} else psQueue.shift()?.resolve(msg);
			} catch {
	...
	psProc.on("exit", (code) => {
		...
		psProc = null;
		psReady = false;
		const err = /* @__PURE__ */ new Error("PowerShell server exited");
...
async function printTicket(payload) {
	...
	if (!psProc) startPs();
	if (!psReady) await new Promise((resolve, reject) => {
		const deadline = setTimeout(() => reject(/* @__PURE__ */ new Error("PowerShell server warmup timed out (10 s)")), 1e4);
...
ipcMain.handle("terminal:print", async (_event, payload) => {
	try {
